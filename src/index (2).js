...
- فقط بر اساس OHLC و داده‌های ارائه‌شده.`;
      ...
      if (url.pathname === "/api/wallet/request_deposit" && request.method === "POST") {
        ...
        try{
          const targets = adminUserIdTargets(env);
...
async async function getOfferConfig(env){
...
// Referral commission (default 10% + per-user override) with max_rewarded_referrals
if(st.referrerId){
  try{
    ...
    if(refSt.rewardedReferralsCount < maxRewarded){
      ...
      const pct = Number.isFinite(Number(refSt.refCommissionPctOverride)) ? Number(refSt.refCommissionPctOverride) : basePct;
...
if(cmd==="/analysis" || text===BTN.ANALYZE){
  if(!isOnboardComplete(st)){
    await tgSendMessage(env, chatId, "برای دریافت تحلیل، ابتدا پروفایل را تکمیل کن ✅", mainMenuKeyboard(env));
    await startOnboardingIfNeeded(env, chatId, from, st);
    return;
  }
  st.state="choose_symbol"; st.selectedSymbol="";
  await saveUser(userId, st, env);
  return tgSendMessage(env, chatId, "📊 دسته‌بندی بازارها برای تحلیل:", signalsMenuKeyboard(env));
}
...
return;
    ...
    if(cmd==="/level" || cmd==="/leveltest" || text===BTN.LEVEL){
...
if(cmd==="/customprompt"){
  ...
  await saveUser(userId, st, env);
  return tgSendMessage(env, chatId, CUSTOM_PROMPT_INFO_TEXT, kb([[BTN.BACK, BTN.HOME]]));
}
...
if(cmd==="/setrefpct"){
  ...
  return tgSendMessage(env, chatId, `✅ تنظیم شد.\nuser=${targetId}\noverride=${pct===0?"(cleared → default)":pct+"%"}`, mainMenuKeyboard(env));
    ...
    if(st.state==="custom_prompt_desc"){
      ...
      if(desc.length < 10) return tgSendMessage(env, chatId, "متن خیلی کوتاه است. لطفاً دقیق‌تر توضیح بده.", kb([[BTN.BACK,BTN.HOME]]));
...
اگر خواستی دوباره تعیین سطح بدی: /level`;
    ...
    if(text===BTN.SET_STYLE){ st.state="set_style"; await saveUser(userId, st, env); return tgSendMessage(env, chatId, "🎯 سبک:", optionsKeyboard(["اسکالپ","سوئینگ","اسمارت‌مانی"])); }
    ...
    if(st.state==="set_style"){ const v=sanitizeStyle(text); if(!v) return tgSendMessage(env, chatId, "یکی از گزینه‌ها را انتخاب کن:", optionsKeyboard(["اسکالپ","سوئینگ","اسمارت‌مانی"])); st.style=v; st.state="idle"; await saveUser(userId, st, env); return tgSendMessage(env, chatId, `✅ سبک: ${st.style}`, mainMenuKeyboard(env)); }
    ...
    if(isSymbol(text)){
      ...
      if(env.BOT_KV && !isPrivileged(from, env)){
        ...
        if(!ok){
          ...
          return tgSendMessage(env, chatId, `⛔ سهمیه امروزت تموم شده. ( ${st.dailyUsed}/${limit} )\n\nبرای افزایش سهمیه، از «${BTN.PAY}» اشتراک بگیر.`, kb([[BTN.PAY],[BTN.HOME],[miniappKey(env)]]));
        }
      }
...
function normalizeSymbol(t){
  ...
  // allow inputs like BTC/USDT or EURUSD, remove common separators
  t = t.replace(/\s+/g,"").replace(/[\-_/]/g,"").replace(/[^A-Z0-9\.]/g,"");
  if(!t) return "";
  ...
  // Some stock tickers may come as AAPL.US or similar; keep dots for mapping arrays
  return t;
}
  ...
  <div class="wrap">
    ...
    <div class="card hidden" id="adminCard">
      ...
      <div class="row" style="margin-top:10px">
        ...
        <div class="col">
          <label>عکس بنر آفر (URL تصویر)</label>
          <input id="admOfferImage" placeholder="https://.../banner.jpg" />
        </div>
      </div>
      <div class="row" style="margin-top:10px">
        <div class="col">
...
<div>
  <h4 style="margin:0 0 8px 0">💸 تغییر کمیسیون رفرال کاربر</h4>
  <div class="row">
    <div class="col">
      <label>User ID</label>
      <input id="commUserId" placeholder="مثلاً 123456789" />
    </div>
    <div class="col">
      <label>درصد کمیسیون (0 تا 100)</label>
      <input id="commPct" placeholder="مثلاً 15" />
    </div>
    <div class="col">
      <label>&nbsp;</label>
      <button id="commSaveBtn" class="primary">ذخیره</button>
    </div>
    <div class="col">
      <label>&nbsp;</label>
      <button id="commClearBtn" class="danger">حذف Override</button>
    </div>
  </div>
  <div class="muted" id="commHint" style="margin-top:8px">اگر Override تنظیم باشد، برای پرداخت‌های زیرمجموعه این کاربر اعمال می‌شود.</div>
</div>
  ...
  async function boot(){
    ...
    if(j.role?.privileged){
      ...
      if(a?.ok){
        ...
        $("admOfferImage").value = a.config.offer?.image || "";
        if(!j.role?.admin){
...
" + j.debug : "");
  ...
  $("saveAdminBtn").addEventListener("click", async ()=>{
    ...
    const payload = {
      ...
      offer: { enabled: $("admOfferEnabled").value === "1", text: $("admOfferText").value, url: $("admOfferUrl").value, image: $("admOfferImage").value },
    };
    if(user?.role?.admin){
...
$("commSaveBtn").addEventListener("click", async ()=>{
  const initData = tg?.initData || "";
  const userId = ($("commUserId").value||"").trim();
  const pct = ($("commPct").value||"").trim();
  if(!userId){ toast("User ID را وارد کن"); return; }
  if(pct === ""){ toast("درصد را وارد کن"); return; }
  const j = await api("/api/admin/commission_set", { initData, userId, pct });
  if(j?.ok){
    toast("ذخیره شد");
  }else{
    toast("ذخیره انجام نشد");
  }
});

$("commClearBtn").addEventListener("click", async ()=>{
  const initData = tg?.initData || "";
  const userId = ($("commUserId").value||"").trim();
  if(!userId){ toast("User ID را وارد کن"); return; }
  const j = await api("/api/admin/commission_set", { initData, userId, pct: null });
  if(j?.ok){
    toast("حذف شد");
    $("commPct").value = "";
  }else{
