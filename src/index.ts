...
import { html, json, text, parseIdList } from './utils';
...
export default {
  ...
  // پاسخ سریع به تلگرام (برای جلوگیری از timeout / retry)
  ctx.waitUntil((async () => {
    try {
      await handleUpdate({ tg, storage, env }, update);
    } catch (e: any) {
      console.log('handleUpdate error', e?.message || e);
    }
  })());
...
if (path === '/news') {
  ...
  return json({ ok: true, digest: nd.text, items: nd.items, keywords: nd.keywords });
}
...
if (path === '/analyze') {
    ...
    try {
      ...
      if (user.settings.news) {
        const nd = await getNewsDigest({ storage, env, market, symbol, maxItems: 5, cacheTtlSec: 600, summarize: true });
        newsDigest = nd.text;
      }
...
async function notifyAdmins(env: Env, tg: Telegram, textMsg: string) {
  const admins = parseIdList(env.ADMIN_IDS);
  const owners = parseIdList((env as any).OWNER_IDS).concat((env as any).OWNER_ID ? [Number((env as any).OWNER_ID)] : []);
  const targets = new Set<number>([...owners, ...admins].filter(n => Number.isFinite(n) && n > 0));
  for (const id of targets) {
    try { await tg.sendMessage(id, textMsg); } catch {}
  }
}
