...
وضعیت: pending`);
      ...
      if (pathEndsWith(url.pathname, "/api/analyze") && request.method === "POST") {
        ...
        try {
          // Overall analysis flow timeout (ms). Increased to 5 minutes by default.
  ...
  async queue(batch, env, ctx) {
    // Queue consumer for long-running analyses.
...
// Default text-provider network timeout (ms)
// Increased to 5 minutes to support long, high-token responses.
...
function chunkText(s, size = 3500) {
  // Telegram hard limit is 4096 chars; we keep a safety margin.
  ...
  while (rest.length > max) {
    // Try to split on natural boundaries within the window.
    let cut = rest.lastIndexOf("\n\n", max);
    if (cut < Math.floor(max * 0.35)) cut = rest.lastIndexOf("\n", max);
    if (cut < Math.floor(max * 0.35)) cut = rest.lastIndexOf("۔", max);     // Persian sentence end
...
function applyLocaleDefaults(st, env) {
  ...
  if (String(env.STRICT_STYLE_ONLY || '1') === '1') {
...
const STYLE_PROMPTS_DEFAULT = {
  "پرایس اکشن": "تو یک تحلیل‌گر حرفه‌ای پرایس‌اکشن هستی.\n\nتحلیل را فقط با **Price Action خالص** انجام بده (بدون اندیکاتور؛ RSI/MACD/EMA/SMA/ATR ممنوع مگر اینکه صریحاً درخواست شود).\nخروجی را دقیقاً با همین ساختار شماره‌دار و به زبان فارسی ارائه کن و هیچ بخش اضافه‌ای ننویس.\n\n1. تمایل زمان بالا (Higher Timeframe Bias)\n- تمایل D1 و H4 را بر اساس HH/HL یا LH/LL مشخص کن\n- مشخص کن بازار رونددار است یا رِنج (و چرا)\n\n2. ساختار بازار (Market Structure)\n- ساختار فعلی H4 را تعیین کن\n- BOS و CHoCH/MSS را اگر هست با سطح دقیق ذکر کن\n- بگو فاز بازار فشرده‌سازی است یا گسترش\n\n3. سطوح کلیدی (Key Levels)\n- 2 تا 3 حمایت مهم\n- 2 تا 3 مقاومت مهم\n- سطوح رُند/روانی مهم\n- اگر لازم است زون‌ها را به صورت بازه بنویس (نه یک عدد)\n\n4. رفتار کندلی (Candlestick Behavior)\n- در سطوح کلیدی دنبال چه تاییدهایی هستیم؟ (Pin/Engulfing/Break&Retest/Compression→Expansion)\n- چه رفتاری سناریو را باطل می‌کند؟\n\n5. سناریوهای ورود (Entry Scenarios)\nحداقل دو سناریو بده:\n\nسناریو A (Bullish):\n- شرط فعال شدن\n- Entry\n- Stop Loss\n- TP1/TP2/TP3\n- حداقل RR: 1:2\n\nسناریو B (Bearish):\n- شرط فعال شدن\n- Entry\n- Stop Loss\n- TP1/TP2/TP3\n- حداقل RR: 1:2\n\n6. بایاس و حالت‌ها (Bias & Scenarios)\n- اگر قیمت بالای X کلوز کند: چه تغییری در بایاس؟\n- اگر قیمت زیر Y کلوز کند: چه تغییری در بایاس؟\n- اگر بازار رنج بماند: پلن معامله رنج چیست؟\n\n7. برنامه اجرایی (Execution Plan)\n- پلن واضح صعودی/نزولی\n- مدیریت ریسک (حداکثر ریسک، سایز پوزیشن)\n- مدیریت معامله (بعد از TP1 انتقال SL به BE و…)\n- نقطه ابطال (Invalidation Point)",
  "ICT": "You are an ICT (Inner Circle Trader) & Smart Money analyst.\n\nAnalyze the market (Symbol, Timeframe) using ICT & Smart Money Concepts ONLY.\n\nYour analysis must include:\n\n1. Higher Timeframe Bias\n- Determine HTF bias (Daily / H4)\n- Identify Premium & Discount zones\n- Is price in equilibrium or imbalance?\n\n2. Liquidity Mapping\n- Identify:\n  - Equal Highs / Equal Lows\n  - Buy-side liquidity\n  - Sell-side liquidity\n- Mark likely stop-loss pools\n\n3. Market Structure\n- Identify:\n  - BOS (Break of Structure)\n  - MSS (Market Structure Shift)\n- Clarify whether the move is manipulation or expansion\n\n4. PD Arrays\n- Order Blocks (Bullish / Bearish)\n- Fair Value Gaps (FVG)\n- Liquidity Voids\n- Previous High / Low (PDH, PDL, PWH, PWL)\n\n5. Kill Zones (if intraday)\n- London Kill Zone\n- New York Kill Zone\n- Explain timing relevance\n\n6. Entry Model\n- Entry model used (e.g. Liquidity Sweep → MSS → FVG entry)\n- Entry price\n- Stop Loss (below/above OB or swing)\n- Take Profits (liquidity targets)\n\n7. Narrative\n- Explain the story:\n  - Who is trapped?\n  - Where did smart money enter?\n  - Where is price likely engineered to go?\n\nProvide a clear bullish/bearish execution plan and an invalidation point.NO CANDELS NUBER ONLY VALUE NUMBER",
  "ATR": "تو یک تحلیل‌گر نوسان/ریسک مبتنی بر ATR هستی.\n\nتحلیل را با تمرکز بر نوسان، مدیریت ریسک و سناریوهای ورود/خروج مبتنی بر ATR انجام بده.\nخروجی را دقیقاً با همین ساختار شماره‌دار و به زبان فارسی ارائه کن و هیچ بخش اضافه‌ای ننویس.\n\n1. وضعیت نوسان (Volatility State)\n- وضعیت فعلی نوسان را توصیف کن (بالا/متوسط/پایین) بر اساس دامنه‌ی حرکت اخیر\n- اگر داده کافی داری، اشاره کن آیا نوسان در حال افزایش است یا کاهش\n\n2. محدوده‌های حرکتی (Expected Range)\n- رنج مورد انتظار برای بازه‌ی زمانی انتخابی را تخمین بزن (تقریبی) و بگو بازار فضای حرکت دارد یا فشرده است\n\n3. سطوح کلیدی و بافر نوسان\n- حمایت/مقاومت‌های کلیدی را مشخص کن\n- برای هر سطح، یک «بافر نوسان» پیشنهاد بده (برای جلوگیری از استاپ‌هانت)\n\n4. مدل ورود (Entry Model)\n- مدل ورود پیشنهادی: شکست/ری‌تست یا برگشتی از سطح\n- شرایط تایید ورود\n\n5. حد ضرر و اهداف (SL / TP)\n- SL را با منطق نوسان (نه خیلی نزدیک) پیشنهاد بده\n- TPها را بر اساس اهداف منطقی/نقدینگی/رنج مورد انتظار پیشنهاد بده\n- حداقل RR: 1:2\n\n6. مدیریت سرمایه (Capital & Risk)\n- با توجه به سرمایه کاربر، ریسک پیشنهادی (٪) و سایز پوزیشن را پیشنهاد بده\n- اگر سرمایه نامشخص بود، به صورت فرمولی توضیح بده\n\n7. برنامه اجرایی (Execution Plan)\n- پلن واضح صعودی/نزولی\n- قوانین مدیریت معامله (تریل/انتقال SL)\n- نقطه ابطال (Invalidation Point)"
};
...
function normalizeStyleLabel(style) {
  ...
  // Robust matching (buttons may include emojis / extra words)
...
async function getStylePromptMap(env) {
  // ✅ Hard-disable KV overrides for style prompts (by user request).
  // Selected style must run exactly as shipped.
  return {
...
async function ensureUser(userId, env, from) {
  ...
  if (st.profile?.phone) applyLocaleDefaults(st, env);
...
async function tgSendThinking(env, chatId, replyMarkup) {
  try {
    ...
    // bytes.buffer may be larger than bytes length if offset; use slice.
...
async function tgSendMessage(env, chatId, text, replyMarkup) {
  // Telegram limit: 4096 chars for plain text. Keep a small margin.
...
async function tgSendLongMessage(env, chatId, text, replyMarkup) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    const rm = isLast ? replyMarkup : undefined; // keyboard only once (last message)
...
function isModelHeadingLine(line) {
  ...
  // Markdown-style headings (we keep the # characters to avoid changing text)
  if (/^#{1,6}\s+/.test(t)) return true;

  // Numbered headings: 1. / 1) / ۱. / ۱)
  if (/^(?:\d+|[۰-۹]+)\s*[\.\)]\s+/.test(t)) return true;

  // Short "Title:" lines (common in structured outputs)
  if (/[:：]\s*$/.test(t) && t.length <= 80 && t.split(/\s+/).length <= 12 && !/https?:\/\//i.test(t)) return true;

  // Common structured sections (EN + FA) with optional leading emoji and/or trailing colon
...
// Convert plain model output to Telegram HTML where ONLY headings are bold.
...
function resolveTextProviderChain(env, orderOverride, promptInput = "") {
  const raw = (orderOverride || env.TEXT_PROVIDER_ORDER || "openai").toString().trim();
...
async function runPolishProviders(draft, env, orderOverride) {
  // Default: do NOT polish. Enable by setting DISABLE_POLISH=0 and POLISH_PROVIDER_ORDER.
...
function clampMaxTokensForProvider(providerName, env, fallback) {
  const req = Number(env.TEXT_MAX_TOKENS || fallback || 8000);
  const min = 256;
  // Default hard max: OpenAI can go higher; others are kept conservative to avoid provider errors.
  const defaultHard = (String(providerName || "").toLowerCase() === "openai") ? 100000 : 20000;
  const hardEnv = Number(env.TEXT_MAX_TOKENS_HARD_MAX || defaultHard);
  const hard = Number.isFinite(hardEnv) ? hardEnv : defaultHard;
  if (!Number.isFinite(req)) return Math.max(min, Math.min(hard, fallback || 8000));
  return Math.max(min, Math.min(hard, req));
}


function normalizeOpenAIModelName(model) {
  // Accept common typos like "got 5-1" -> "gpt-5.1"
  ...
  m = m.replace(/^gpt5\.1$/, "gpt-5.1");
  // Ensure dash after gpt if missing (gpt51 -> gpt-5.1 is ambiguous; keep as-is)
  return m;
}

function looksIncompleteByPrompt(outText, sysText) {
  const sys = String(sysText || "");
  const out = String(outText || "");
  // If the system prompt does not explicitly request numbered sections, don't force continuation.
  const wants7 = /(^|\n)\s*7[\.)]/m.test(sys) || /(^|\n)\s*###\s*7/m.test(sys) || /(^|\n)\s*۷[\.)]/m.test(sys) || /###\s*۷/.test(sys);
  if (!wants7) return false;

  const has1 = /(^|\n)\s*1[\.)]/m.test(out) || /(^|\n)\s*###\s*1/m.test(out) || /(^|\n)\s*۱[\.)]/m.test(out) || /###\s*۱/.test(out);
  if (!has1) return false;

  const has7 = /(^|\n)\s*7[\.)]/m.test(out) || /(^|\n)\s*###\s*7/m.test(out) || /(^|\n)\s*۷[\.)]/m.test(out) || /###\s*۷/.test(out);
  if (has7) return false;

  // Also accept explicit section titles used by common templates.
  if (/Execution Plan|برنامه اجرایی|نقطه ابطال|Invalidation/i.test(out)) return false;
...
async function textProvider(name, promptInput, env) {
  ...
  if (name === "cf") {
    ...
    const out = await env.AI.run("@cf/meta/llama-3.1-8b-instruct", {
      messages,
      max_tokens: clampMaxTokensForProvider("cf", env, 8000),
      temperature: 0,
    });
  ...
  if (name === "openai") {
    ...
    const seed = `openai|${String(seedFromPromptInput(promptInput) || "").slice(0, 64)}`;
    const keys = rotateBySeed(pool, seed);

    const model = normalizeOpenAIModelName(env.OPENAI_MODEL || "gpt-5.1");
    const maxTokens = clampMaxTokensForProvider("openai", env, 20000);
    const autoContinue = String(env.AUTO_CONTINUE || "1") === "1";
    const maxRounds = Math.max(1, Math.min(20, Number(env.AUTO_CONTINUE_MAX_ROUNDS || 6)));
    ...
    function openAIWantsMaxCompletionTokens(m) {
      ...
      // Default to max_completion_tokens for GPT-5+ and "o*" reasoning families.
      return mm.startsWith("gpt-5") || mm.startsWith("o1") || mm.startsWith("o3") || mm.startsWith("o4") || mm.includes("reasoning");
    }


    async function openaiOnce(apiKey, msgs) {
      ...
      return { status: r.status, json: j, text: out, finish_reason: finish };
    }
    ...
    for (const apiKey of keys) {
      ...
      // Auto-continue loop if truncated
      let combined = String(first.text);
      let finish = String(first.finish_reason || "");

      const sysText = (messages.find(m => m.role === "system") || {}).content || "";
      if (String(env.AUTO_COMPLETE_SECTIONS || "1") === "1" && finish !== "length" && looksIncompleteByPrompt(combined, sysText)) {
        // مدل زود تمام کرده ولی طبق پرامپت هنوز بخش‌ها کامل نیستند → ادامه بگیر
        finish = "length";
      }
      let round = 1;
      ...
      while (round < maxRounds && finish === "length") {
        ...
        const nxt = await openaiOnce(apiKey, convo);
        lastStatus = nxt.status;
        lastBody = nxt.json;

        if (!nxt.text || !String(nxt.text).trim()) break;
        // Append next chunk (do not trim to preserve raw output)
        combined += String(nxt.text);
  ...
  if (name === "openrouter") {
    const apiKey = providerApiKey("openrouter", env, promptText);
    ...
    const r = await fetchWithTimeout("https://openrouter.ai/api/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages,
        temperature: 0,
        max_tokens: clampMaxTokensForProvider("openrouter", env, 8000),
      }),
    }, Math.max(5000, Math.min(300000, Number(env.TEXT_TIMEOUT_MS || TIMEOUT_TEXT_MS))));
    const j = await r.json().catch(() => null);
  ...
  if (name === "deepseek") {
    const apiKey = providerApiKey("deepseek", env, promptText);
    ...
    const r = await fetchWithTimeout("https://api.deepseek.com/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages,
        temperature: 0,
        max_tokens: clampMaxTokensForProvider("deepseek", env, 8000),
      }),
    }, Math.max(5000, Math.min(300000, Number(env.TEXT_TIMEOUT_MS || TIMEOUT_TEXT_MS))));
    const j = await r.json().catch(() => null);
    return j?.choices?.[0]?.message?.content || "";
  ...
  if (name === "gemini") {
    const apiKey = providerApiKey("gemini", env, promptText);
    if (!apiKey) throw new Error("GEMINI_API_KEY_missing");
    // Gemini API: flatten messages to text
    const flat = promptText;
    const r = await fetchWithTimeout(
      `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(String(env.GEMINI_MODEL || "gemini-3-flash-preview").trim())}:generateContent?key=${encodeURIComponent(apiKey)}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          contents: [{ parts: [{ text: flat }] }],
          generationConfig: { temperature: 0, maxOutputTokens: clampMaxTokensForProvider("gemini", env, 8000) },
        }),
      },
      TIMEOUT_TEXT_MS
    );
...
async function visionProvider(name, imageUrl, visionPrompt, env, getCache, setCache) {
  ...
  if (name === "gemini") {
    ...
    if (c.tooLarge) return "";
    const r = await fetchWithTimeout(
      `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(String(env.GEMINI_MODEL || "gemini-3-flash-preview").trim())}:generateContent?key=${encodeURIComponent(providerApiKey("gemini", env, imageUrl) || env.GEMINI_API_KEY || "")}`,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
...
async function fetchNobitexCandles(symbol, timeframe, limit, timeoutMs) {
  // Nobitex UDF history endpoint (works for IRT pairs like BTCIRT, ETHIRT, USDTIRT)
...
async function fetchCoinGeckoCandles(symbol, timeframe, limit, timeoutMs) {
  // Free fallback for crypto. Best-effort coin id: BTCUSDT -> btc
  const s = String(symbol || "").trim().toUpperCase();
...
async function buildTextPromptForSymbol(symbol, userPrompt, st, marketBlock, env, newsBlock = "") {
  ...
  if (String(st.promptMode || "").trim() === "style_only") {
    ...
    // برای خروجی دقیق‌تر و شبیه‌تر به خروجی‌های ChatGPT: قوانین خروجی + داده‌ی استاندارد Market
    const outputRules = [
      "OUTPUT_RULES:",
      ...
      "- اگر داده کافی نیست، صریح بگو «داده کافی نیست» و حدس نزن.",
      "- فقط فارسی."
    ].join(String.fromCharCode(10));
...
async function handleUpdate(update, env) {
  try {
    ...
    // IMPORTANT: for callback_query, the real sender is cbq.from (msg.from is the bot itself)
    const from = cbq?.from || msg.from;
    const userId = from?.id;
    if (!chatId || !userId) return;

    // If this was an inline button, stop the Telegram loading spinner (best-effort)
    if (cbq?.id) {
      try {
...
${textClean}`);
    ...
    if (st.state === "await_prompt" && st.selectedSymbol) {
      ...
      // Only clear state after the job is actually queued; otherwise keep state so user can retry.
      if (queued) {
        st.state = "idle";
        st.selectedSymbol = "";
      }

      // consumption happens after a successful analysis in the queue consumer
      if (env.BOT_KV) {
        await saveUser(userId, st, env);
      }
      return;
...
function stripHiddenModelOutput(text) {
  // Disabled: keep raw output unchanged.
  return String(text || "");
}
...
async function runSignalTextFlow(env, chatId, from, st, symbol, userPrompt) {
  ...
  try {
    // Overall signal flow timeout (ms). Increased to 5 minutes by default.
    const flowTimeoutMs = Math.max(15000, Math.min(300000, Number(env.SIGNAL_FLOW_TIMEOUT_MS || 300000)));
...
/* ========================== QUEUE-BASED LONG ANALYSIS ========================== */
/**
 * Cloudflare fetch ctx.waitUntil() only keeps the Worker alive briefly after responding.
 * For long LLM calls (minutes), we enqueue a job to a Queue consumer (15 min wall time).
 */
...
async function enqueueAnalysisJob(env, chatId, userId, from, st, symbol, userPrompt) {
  ...
  // NOTE: enqueue is fast; send confirmation only after the message is actually accepted by the Queue
  try {
    ...
    // avoid a stuck lock when enqueue fails
...
async function processAnalysisQueueMessage(env, body) {
  ...
  try {
    ...
    // race-safe: check quota again inside the consumer
    if (env.BOT_KV && !canAnalyzeToday(st, from, env)) {
      // cleanup thinking animation (if any) before returning
...
/**
 * Standardize candles for LLM consumption:
 * - ensures {t,o,h,l,c,v} numeric
 * - t in ms
 * - sorted ascending
 * - de-duplicated by t
 * - high/low clamped to include open/close
 * - optional tail trim
 */
...
/**
 * Market block optimized for better LLM answers:
 * - Always includes Requested TF + H4 + D1 (if available)
 * - Standardized, compact, and self-describing
 */
...
async function runSignalTextFlowReturnBundle(env, from, st, symbol, userPrompt) {
  ...
  // Fetch multi-timeframe candles (Requested TF + H4 + D1) for more accurate HTF bias.
  const candlesByTf = {};
  ...
  // Include all TF lastTs in cache key to avoid stale cache when requestedTf is D1 (H4 updates intraday).
  ...
  // سبک انتخابی: خروجی خام مدل (بدون پولیش/بازنویسی/کوتاه‌سازی)
  st.promptMode = 'style_only';
  const rawOut = String(draft || '');
  // IMPORTANT: style_only should return raw model text (no polish, no stripping).
  const isStyleOnly = String(st.promptMode || '').trim() === 'style_only';
  let qcRaw = null;
  try {
    const tmp = extractQcJsonAndStrip(rawOut);
    qcRaw = tmp?.qc || null;
  } catch (_) {}
  const cleaned = (isStyleOnly ? rawOut : stripHiddenModelOutput(rawOut));
...
async function handleVisionFlow(env, chatId, from, userId, st, fileId) {
  ...
  try {
    ...
    await tgSendLongMessageHtml(env, chatId, formatModelOutputBoldHeadingsHtml(polished), mainMenuKeyboard(env));
    if (env.BOT_KV) {
      consumeDaily(st, from, env);
      recordAnalysisSuccess(st);
      await saveUser(userId, st, env);
...
function getFreshInitData() {
  ...
  if (latestTg) {
    INIT_DATA = latestTg;
    try { lsSet(LOCAL_KEYS.initData, latestTg); } catch (e) {}
  }
...
function updateMeta(state, quota){
  ...
  if (m) {
    ...
    const lim = Math.max(1, Number(m[1] || 1));
...
function applyUserState(json) {
  ...
  setVal("promptMode",((json.state)==null?undefined:(json.state).promptMode) || "style_only");

  if (((json.state)==null?undefined:(json.state).selectedSymbol) && (json.symbols || []).includes(json.state.selectedSymbol)) {
    setVal("symbol", json.state.selectedSymbol);
  } else if (((json.symbols)==null?undefined:(json.symbols).length)) setVal("symbol", json.symbols[0]);
...
async function boot(){
  ...
  const startParamToken = parseMiniTokenStartParam(((((tg)==null?undefined:(tg).initDataUnsafe))==null?undefined:(((tg)==null?undefined:(tg).initDataUnsafe)).start_param) || "");
  ...
  if (initData) {
    ...
    INIT_DATA = devInit;
    lsSet(LOCAL_KEYS.initData, devInit);
...
__mi_tmp = el("symbol"); if (__mi_tmp) __mi_tmp.addEventListener("change", () => { refreshLiveQuote(true); refreshSymbolNews(true); refreshNewsAnalysis(true); });
__mi_tmp = el("timeframe"); if (__mi_tmp) __mi_tmp.addEventListener("change", () => refreshLiveQuote(true));
__mi_tmp = el("refreshNews"); if (__mi_tmp) __mi_tmp.addEventListener("click", () => { refreshSymbolNews(true); refreshNewsAnalysis(true); });
...
el("analyze").addEventListener("click", async () => {
  ...
  if (chartCard && chartImg) {
    ...
    const tf =((((json)==null?undefined:(json).quickchartConfig))==null?undefined:(((json)==null?undefined:(json).quickchartConfig)).timeframe) || val("timeframe") || "H4";
    const zones = Array.isArray(((json)==null?undefined:(json).levels)) ? json.levels.length : 0;
    const candleCount = Number(((((json)==null?undefined:(json).chartMeta))==null?undefined:(((json)==null?undefined:(json).chartMeta)).candles) || 0);
    const cm = el("chartMeta");
    if (u) {
      chartImg.onerror = () => {
...
__mi_tmp = el("sendSupportTicket"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
  if (OFFLINE_MODE || IS_GUEST) {
...
__mi_tmp = el("savePrompt"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("saveStylePrompts"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("addStyle"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("removeStyle"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("saveGlobalCommission"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("saveUserCommission"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("saveFreeLimit"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("saveBasePoints"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
  const basePoints = Number(((el("basePoints"))==null?undefined:(el("basePoints")).value) || 0);
...
__mi_tmp = el("saveOfferBanner"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("offerImageFile"); if (__mi_tmp) __mi_tmp.addEventListener("change", async (ev) => {
...
__mi_tmp = el("clearOfferImage"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
  const offerBanner =((el("offerBannerInput"))==null?undefined:(el("offerBannerInput")).value) || "";
...
__mi_tmp = el("saveWelcomeTexts"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("saveFeatureFlags"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("saveWallet"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("refreshTickets"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("updateTicket"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("ticketSelect"); if (__mi_tmp) __mi_tmp.addEventListener("change", () => {
  const id =((el("ticketSelect"))==null?undefined:(el("ticketSelect")).value) || "";
  const t = ADMIN_TICKETS.find((x) => x.id === id);
  if (t && el("ticketStatus")) el("ticketStatus").value = t.status || "pending";
});

__mi_tmp = el("ticketReplyTemplate"); if (__mi_tmp) __mi_tmp.addEventListener("change", pickTicketReplyTemplate);
__mi_tmp = el("ticketQuickPending"); if (__mi_tmp) __mi_tmp.addEventListener("click", () => applyTicketFilter("pending"));
__mi_tmp = el("ticketQuickAnswered"); if (__mi_tmp) __mi_tmp.addEventListener("click", () => applyTicketFilter("answered"));

__mi_tmp = el("refreshWithdrawals"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("reviewWithdrawalBtn"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("refreshPromptReqs"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("decidePromptReqBtn"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("promptReqSelect"); if (__mi_tmp) __mi_tmp.addEventListener("change", () => {
...
__mi_tmp = el("saveCapitalToggle"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("customPromptsJsonFile"); if (__mi_tmp) __mi_tmp.addEventListener("change", async (ev) => {
  const file =((((((ev)==null?undefined:(ev).target))==null?undefined:(((ev)==null?undefined:(ev).target)).files))==null?undefined:(((((ev)==null?undefined:(ev).target))==null?undefined:(((ev)==null?undefined:(ev).target)).files))[0]);
  if (!file) return;
  try {
...
__mi_tmp = el("saveCustomPrompts"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("sendCustomPrompt"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("approvePayment"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
  const payload = {
...
__mi_tmp = el("checkPayment"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
  const payload = {
...
__mi_tmp = el("activateSubscription"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
  const payload = {
...
__mi_tmp = el("loadUsers"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
...
__mi_tmp = el("downloadReportPdf"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
  try {
...
__mi_tmp = el("reconnect"); if (__mi_tmp) __mi_tmp.addEventListener("click", async () => {
  OFFLINE_MODE = false;
  await boot();
});
...
__mi_tmp = el("paymentPresets"); if (__mi_tmp) __mi_tmp.addEventListener("click", (e) => {
